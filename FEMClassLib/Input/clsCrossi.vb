'This file contains all the methods of crossi.f
Partial Public Class clsInput
    ...
    ''' </summary>
    ''' <param name="x"></param>
    Public Sub crossi(ByVal x As Double(,), ByRef llc As Integer, ByRef lresrt As Integer, ByRef resltl As Double, ByRef result As Double, ByVal i As Integer, ByVal k As Integer, ByVal m As Integer, ByVal n As Integer)
        Dim c11 = x(1, m) - x(1, k)
        Dim c12 = x(2, m) - x(2, k)
        Dim c13 = x(3, m) - x(3, k)
        Dim c21 = x(1, n) - x(1, k)
        ...
        If (resltl <= dprdf) Then
            lresrt = llc
            resltl = dprdf
        End If
        Dim resul1 = c1dc1 * sldc2 - c1dc2 * sldc1
        Dim resul2 = sldc1 * sldc2 - c1dc2 * sldsl
        Dim resul3 = c2dc2 * sldc1 - c1dc2 * sldc2
        result = -1.0
        If Math.Min(Math.Min(resul1, resul2), resul3) < 0.0 Then Return
        result = 1.0
    End Sub
